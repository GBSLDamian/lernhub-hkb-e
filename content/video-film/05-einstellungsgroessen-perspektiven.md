---
id: einstellungsgroessen-perspektiven
area: video-film
titel_de: Einstellungsgrössen & Perspektiven
titel_fr: Cadrages & perspectives
reihenfolge: 5
lernfeld: LF3
lehrjahr: 1
typ: theorie
kstufe: K3
lernziele_de:
  - Ich benenne die gängigen Einstellungsgrössen und ihre Wirkung.
  - Ich wähle eine Kameraperspektive passend zur gewünschten Wirkung.
lernziele_fr:
  - Je nomme les cadrages courants et leur effet.
  - Je choisis une perspective de caméra adaptée à l'effet recherché.
tags: [einstellungsgroessen, perspektiven]
---

:::de
Dieselbe Person, derselbe Ort — nur die Kamera zoomt immer weiter rein. Eine Einstellungsgrösse bestimmt, wie viel von einer Person oder Umgebung im Bild zu sehen ist, und wird eingesetzt, um bestimmte Emotionen und Reaktionen beim Publikum hervorzurufen.
:::
:::fr
La même personne, le même endroit — seule la caméra zoome de plus en plus près. Un cadrage détermine la part visible d'une personne ou d'un environnement à l'image, et sert à susciter certaines émotions et réactions chez le public.
:::

:::h2 de="Von der Supertotalen bis ins Detail" fr="Du plan très large au très gros plan":::

:::de
**Supertotale:** noch weiter entfernt als die Totale — zeigt Objekt oder Szene in einem sehr breiten Blickfeld und betont die Grösse des Ortes. Wird oft am Anfang oder Ende einer Szene verwendet, um eine Verbindung zur vorherigen oder nächsten Szene herzustellen.
:::
:::fr
**Plan très large (supertotale) :** encore plus éloigné que le plan d'ensemble — montre le sujet ou la scène dans un très large champ de vision et souligne l'ampleur du lieu. Souvent utilisé au début ou à la fin d'une scène pour créer un lien avec la scène précédente ou suivante.
:::

![Totale](/assets/svg/einstellungsgroessen/totale.svg)

:::de
**Totale:** Gesamte Szenerie, Person klein darin — Orientierung, guter Einstieg.
:::
:::fr
**Plan d'ensemble :** toute la scène, la personne y paraît petite — orientation, bonne entrée en matière.
:::

![Halbtotale](/assets/svg/einstellungsgroessen/halbtotale.svg)

:::de
**Halbtotale:** Person komplett von Kopf bis Fuss.
:::
:::fr
**Plan demi-ensemble :** la personne entière, de la tête aux pieds.
:::

:::de
**Amerikanische Einstellung:** zeigt das Objekt von den Knien bis zur Brust — betont meistens Waffen oder Werkzeuge, die die Figur in der Szene verwendet (klassisch aus dem Westernfilm).
:::
:::fr
**Plan américain :** montre le sujet des genoux à la poitrine — met généralement en valeur des armes ou outils utilisés par le personnage dans la scène (classique dans le western).
:::

![Halbnah](/assets/svg/einstellungsgroessen/halbnah.svg)

:::de
**Halbnah:** Person von Kopf bis Hüfte.
:::
:::fr
**Plan rapproché-taille :** la personne de la tête à la hanche.
:::

![Nah](/assets/svg/einstellungsgroessen/nah.svg)

:::de
**Nah:** Person von Kopf bis Brust — klassische Interview-Einstellung.
:::
:::fr
**Plan rapproché-poitrine :** la personne de la tête à la poitrine — cadrage classique d'interview.
:::

![Grossaufnahme](/assets/svg/einstellungsgroessen/gross.svg)

:::de
**Grossaufnahme:** Nur der Kopf/das Gesicht — starke emotionale Wirkung.
:::
:::fr
**Gros plan :** seulement la tête/le visage — fort effet émotionnel.
:::

![Detail](/assets/svg/einstellungsgroessen/detail.svg)

:::de
**Detail:** Nur ein Ausschnitt (Auge, Hand, Objekt).
:::
:::fr
**Très gros plan :** seulement un détail (œil, main, objet).
:::

:::merksatz
:::de
Beim Schneiden nie mehr als eine Einstellungsgrösse «überspringen» — sonst wirkt der Schnitt wie ein Fehler (Jump Cut).
:::
:::fr
Au montage, ne jamais « sauter » plus d'un cadrage — sinon la coupe paraît être une erreur (jump cut).
:::
:::

:::h2 de="Der Blickwinkel der Kamera" fr="L'angle de vue de la caméra":::

![Augenhöhe](/assets/svg/perspektiven/augenhoehe.svg)

:::de
**Normalsicht/Augenhöhe:** Kamera auf Höhe des Motivs, entspricht der natürlichen menschlichen Sicht — neutral, «ehrliche» Wirkung, stellt Vertrautheit mit den Charakteren her.
:::
:::fr
**Vue normale/hauteur des yeux :** caméra au niveau du sujet, correspond à la vision humaine naturelle — effet neutre, « honnête », crée une familiarité avec les personnages.
:::

:::de
**Aufsicht:** eine leicht erhöhte Position über dem Objekt — der Charakter kann kleiner oder verletzlicher wirken, oft in ungewohnten oder beängstigenden Situationen eingesetzt.
:::
:::fr
**Plongée légère :** une position légèrement surélevée par rapport au sujet — le personnage peut paraître plus petit ou vulnérable, souvent utilisée dans des situations inhabituelles ou inquiétantes.
:::

![Vogelperspektive](/assets/svg/perspektiven/vogelperspektive.svg)

:::de
**Vogelperspektive:** eine sehr hohe Position, als würde man von oben aufs Geschehen hinabschauen — wirkt klein, überblickbar, erzeugt Distanz. Oft mit Drohnen aufgenommen.
:::
:::fr
**Plongée totale (vue d'oiseau) :** une position très élevée, comme si on regardait la scène d'en haut — paraît petit, facile à embrasser du regard, crée de la distance. Souvent filmée par drone.
:::

:::de
**Untersicht:** eine leicht niedrige Position unterhalb des Objekts — betont Grösse oder Wichtigkeit, da die Figur von unten grösser und imposanter erscheint. Häufig in actionreichen Szenen.
:::
:::fr
**Contre-plongée légère :** une position légèrement basse par rapport au sujet — souligne la grandeur ou l'importance, le personnage paraissant plus grand et imposant vu d'en bas. Fréquente dans les scènes d'action.
:::

![Froschperspektive](/assets/svg/perspektiven/froschperspektive.svg)

:::de
**Froschperspektive:** eine sehr niedrige Position, als würde man von ganz unten aufschauen — «von unten = Held». Lässt Charaktere mächtig oder bedrohlich erscheinen, oft für übermächtige Figuren wie Superheld:innen oder Bösewichte.
:::
:::fr
**Contre-plongée totale :** une position très basse, comme si on regardait tout en bas vers le haut — « vu d'en bas = héros ». Rend les personnages puissants ou menaçants, souvent pour des figures surpuissantes comme les super-héros ou les méchants.
:::

:::merksatz
:::de
**«Von unten = Held.»** Froschperspektive lässt ein Motiv mächtig wirken — Vogelperspektive macht es klein und überblickbar.
:::
:::fr
**« Vu d'en bas = héros. »** La contre-plongée rend un sujet imposant — la plongée le rend petit et facile à cerner.
:::
:::

:::h2 de="So sieht das in echten Fotos aus" fr="Voici à quoi cela ressemble sur de vraies photos":::

![Luftaufnahme einer Küstenstadt senkrecht von oben fotografiert](/assets/img/film/perspektive-vogel.jpg)

:::de
**Vogelperspektive.** Aus der Luft wirkt alles klein und überblickbar. *(Foto: John D Fielding, CC BY 2.0)*
:::
:::fr
**Vue d'oiseau.** Depuis les airs, tout paraît petit et facile à embrasser du regard. *(Photo : John D Fielding, CC BY 2.0)*
:::

![Blick von ganz unten steil nach oben zu Wolkenkratzern](/assets/img/film/perspektive-frosch.jpg)

:::de
**Froschperspektive.** Von unten fotografiert wirken selbst Gebäude überwältigend gross. *(Foto: CC0)*
:::
:::fr
**Contre-plongée.** Photographiés d'en bas, même les bâtiments paraissent écrasants. *(Photo : CC0)*
:::

:::h2 de="Bild und Wirkung zuordnen" fr="Associer image et effet":::

:::widget einstellung-matcher
{
  "items": [
    { "id": "totale", "img": "/assets/svg/einstellungsgroessen/totale.svg", "nameDe": "Totale", "nameFr": "Plan d'ensemble", "effektDe": "Zeigt die gesamte Szenerie, Person klein darin — gute Orientierung, guter Einstieg.", "effektFr": "Montre toute la scène, la personne y paraît petite — bonne orientation, bonne entrée en matière." },
    { "id": "halbtotale", "img": "/assets/svg/einstellungsgroessen/halbtotale.svg", "nameDe": "Halbtotale", "nameFr": "Plan demi-ensemble", "effektDe": "Person komplett von Kopf bis Fuss sichtbar.", "effektFr": "La personne entière, de la tête aux pieds, est visible." },
    { "id": "halbnah", "img": "/assets/svg/einstellungsgroessen/halbnah.svg", "nameDe": "Halbnah", "nameFr": "Plan rapproché-taille", "effektDe": "Person von Kopf bis Hüfte sichtbar.", "effektFr": "La personne, de la tête à la hanche, est visible." },
    { "id": "nah", "img": "/assets/svg/einstellungsgroessen/nah.svg", "nameDe": "Nah", "nameFr": "Plan rapproché-poitrine", "effektDe": "Person von Kopf bis Brust — klassische Interview-Einstellung.", "effektFr": "La personne, de la tête à la poitrine — cadrage classique d'interview." },
    { "id": "gross", "img": "/assets/svg/einstellungsgroessen/gross.svg", "nameDe": "Grossaufnahme", "nameFr": "Gros plan", "effektDe": "Nur Kopf/Gesicht sichtbar — starke emotionale Wirkung.", "effektFr": "Seuls la tête/le visage sont visibles — fort effet émotionnel." },
    { "id": "detail", "img": "/assets/svg/einstellungsgroessen/detail.svg", "nameDe": "Detail", "nameFr": "Très gros plan", "effektDe": "Nur ein Ausschnitt sichtbar (Auge, Hand, Objekt).", "effektFr": "Seul un détail est visible (œil, main, objet)." }
  ]
}
:::

:::h2 de="Kurz angewendet" fr="Application rapide":::

:::widget quiz
{
  "anzahl": 4,
  "fragen": [
    { "id": "q1", "fragenDe": "Du willst zeigen, wo die Szene überhaupt spielt – die ganze Umgebung soll sichtbar sein.", "fragenFr": "Tu veux montrer où se déroule la scène – tout l'environnement doit être visible.", "optionenDe": ["Totale", "Grossaufnahme", "Froschperspektive", "Detail"], "optionenFr": ["Plan d'ensemble", "Gros plan", "Contre-plongée", "Très gros plan"], "loesung": 0, "erklaerungDe": "Die Totale etabliert den Ort – guter Einstieg in eine Sequenz.", "erklaerungFr": "Le plan d'ensemble établit le lieu – bonne entrée en matière." },
    { "id": "q2", "fragenDe": "Du willst eine sehr emotionale Einstellung: Jede Gefühlsregung im Gesicht soll klar sichtbar sein.", "fragenFr": "Tu veux un cadrage très émotionnel : chaque expression du visage doit être clairement visible.", "optionenDe": ["Halbtotale", "Grossaufnahme", "Vogelperspektive", "Totale"], "optionenFr": ["Plan demi-ensemble", "Gros plan", "Plongée", "Plan d'ensemble"], "loesung": 1, "erklaerungDe": "Die Grossaufnahme zeigt nur den Kopf – starke emotionale Wirkung.", "erklaerungFr": "Le gros plan ne montre que la tête – fort effet émotionnel." },
    { "id": "q3", "fragenDe": "Dein Motiv soll mächtig, imposant und dominant wirken.", "fragenFr": "Ton sujet doit paraître puissant, imposant et dominant.", "optionenDe": ["Vogelperspektive", "Augenhöhe", "Froschperspektive", "Halbnah"], "optionenFr": ["Plongée", "Hauteur des yeux", "Contre-plongée", "Plan rapproché-taille"], "loesung": 2, "erklaerungDe": "«Von unten = Held» – die Froschperspektive lässt Motive mächtig wirken.", "erklaerungFr": "« Vu d'en bas = héros » – la contre-plongée rend les sujets puissants." },
    { "id": "q4", "fragenDe": "Klassische Interview-Situation: Kopf bis Brust sichtbar, Mimik gut lesbar.", "fragenFr": "Situation d'interview classique : tête à poitrine visible, mimique bien lisible.", "optionenDe": ["Detail", "Nah", "Totale", "Vogelperspektive"], "optionenFr": ["Très gros plan", "Plan rapproché-poitrine", "Plan d'ensemble", "Plongée"], "loesung": 1, "erklaerungDe": "Nah ist die klassische Interview-Einstellungsgrösse.", "erklaerungFr": "Le plan rapproché-poitrine est le cadrage classique de l'interview." },
    { "id": "q5", "fragenDe": "Du willst näher an eine Person heran, ohne dass sich die Perspektive unnatürlich verzerrt.", "fragenFr": "Tu veux te rapprocher d'une personne sans que la perspective se déforme de façon non naturelle.", "optionenDe": ["Digital zoomen", "Mit den Füssen näher hingehen (Fahrt)", "Die Kamera schwenken", "Das Bild nachträglich zuschneiden"], "optionenFr": ["Zoomer numériquement", "Se rapprocher à pied (travelling)", "Faire un panoramique", "Recadrer l'image après coup"], "loesung": 1, "erklaerungDe": "Eine echte Fahrt verändert Tiefe und Perspektive natürlich.", "erklaerungFr": "Un vrai travelling modifie la profondeur et la perspective de façon naturelle." }
  ]
}
:::
